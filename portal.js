...
async function fetchLive() {
  ...
  try {
    ...
    Object.values(liveTimers).forEach(t => { if (typeof t === 'number') { clearInterval(t); clearTimeout(t); } });
    liveTimers = {};
    if (sessions.length === 0) { document.getElementById('liveGrid').innerHTML = empty('😴', 'Aucune session active'); return; }
    // Use agent-calculated elapsed/remaining (avoids device clock differences)
    document.getElementById('liveGrid').innerHTML = sessions.map(s => {
      const isFree = s.isFree;
      const init = isFree ? s.elapsedSec : s.remainingSec;
      return `<div class="station-card active"><div class="s-name">📺 ${s.stationName}</div><div class="s-timer" id="lt-${s.sessionId}">${fmt(init)}</div><div class="s-label">${isFree ? '⬆ Libre' : '⬇ Restant'}</div><div style="font-size:10px;color:var(--text2);margin-top:4px;">${s.players||1}J · ${(s.price||0).toFixed(2)} TND</div></div>`;
    }).join('');

    // Count locally from agent values (no Date.now() needed)
    sessions.forEach(s => {
      const el2 = document.getElementById('lt-' + s.sessionId);
      if (!el2) return;
      let counter = s.isFree ? s.elapsedSec : s.remainingSec;
      liveTimers[s.sessionId] = setInterval(() => {
        counter = s.isFree ? counter + 1 : Math.max(0, counter - 1);
        el2.textContent = fmt(counter);
      }, 1000);
    });

    // Re-fetch every 15s to catch libre switches and correct durationSec
    liveTimers['_refresh'] = setTimeout(fetchLive, 15000);
  } catch(e) {
