...
async function fetchLive() {
  ...
  try {
    ...
    document.getElementById('liveGrid').innerHTML = sessions.map(s => {
      // Normalize startedAtEpochMs (TV app may send seconds instead of ms)
      let startMs = s.startedAtEpochMs;
    ...
    sessions.forEach(s => {
      ...
      if (!el2) return;
      const startMsTimer = s.startedAtEpochMs < 1e12 ? s.startedAtEpochMs * 1000 : s.startedAtEpochMs;
      liveTimers[s.sessionId] = setInterval(() => {
        const elapsed = Math.floor((Date.now() - startMsTimer)/1000);
